...
def matching_result_map(dataset_file, task, background_id):
    ...
    elif result_json_path and (dataset_gridod_json_path or dataset_grid_json_path):
        logger.info("The result json path is: " + result_json_path)
        map_save_path = settings.ADMIN_FRONT_HTML_PATH + dataset_file.file_name + "_" + str(task.exp_id) + "_result.html"
        try:
            if dataset_gridod_json_path:
                logger.info("The dataset gridod json path is: " + dataset_gridod_json_path)
                render_grid_to_map(dataset_gridod_json_path, result_json_path, background_id, map_save_path, dataset_dir)
            if dataset_grid_json_path:
...
def render_to_map(dataset_json_path, result_json_path, background_id, map_save_path, dataset_dir):
    ...
    try:
        ...
        if len(prediction_mean[0]) == 2:
            ...
            # 会有多维度的结果 刷成二维
            # data_ndim = prediction_mean.ndim
            # while data_ndim != 2:
            #     prediction_mean = prediction_mean.mean(0)
            #     truth_mean = truth_mean.mean(0)
            #     dif_mean = dif_mean.mean(0)
            #     data_ndim = prediction_mean.ndim
            value1_list_pre, value1_list_truth, value1_list_dif = [], [], []
            if name_list[0] == 'risk':
...
def form_line_statis(value_dict, asix_x, map_save_path, namelist):
    ...
    # elif len(namelist) == 2 and len(value_dict) == 8:
    #     Line(init_opts=opts.InitOpts(width="1200px", height="800px")).add_xaxis(xaxis_data=asix_x). \
    #         add_yaxis("mean of predict " + str(namelist[0]), value_dict[0], markline_opts=opts.MarkLineOpts(
    #         data=[opts.MarkLineItem(type_="average", name="平均值")]), ). \
    #         add_yaxis("mean of predict " + str(namelist[1]), value_dict[1], markline_opts=opts.MarkLineOpts(
...
def render_grid_to_map(dataset_grid_json_path, result_json_path, background_id, map_save_path, dataset_dir):
    ...
    truth = file_data['truth']
    dif = prediction - truth
    logger.info("prediction shape: {}, ndim: {}", prediction.shape, prediction.ndim)
    logger.info("truth shape: {}, ndim: {}", truth.shape, truth.ndim)
    logger.info("dif shape: {}, ndim: {}", dif.shape, dif.ndim)
    try:
        ...
        logger.error("实验结果渲染 render_grid_to_map 异常：{}", ex)
        name_list = return_data_names(dataset_dir)
...
def make_cor(data, m, dataset_json_content, dataset_dir, name):
    ...
    # 把坐标和对应值组装起来
    # data_list = np.array(data_list)
    csv_pd = pd.DataFrame(data=data_list)
    csv_path = dataset_dir + "/form_cor.csv"
    ...
    try:
        ...
        if len(name_list) == 1:
            df = pd.read_csv(csv_path, header=None, names=['geo_id', prefix  + str(name_list[0])])
            df.to_csv(csv_path, index=False)
            business.save_geojson.add_choropleth(csv_path, m, state_geo=dataset_json_content, tag1=str(name_list[0]),
                                                 name=name, tag_prefix=prefix)
        else:
            df = pd.read_csv(csv_path, header=None,
                             names=['geo_id', prefix + 'total_' + str(name_list[0]) + '_' + str(name_list[1])])
            df.to_csv(csv_path, index=False)
            business.save_geojson.add_choropleth(csv_path, m, state_geo=dataset_json_content, tag1=str(name_list[0]),
                                                 tag2=str(name_list[1]), name=name, tag_prefix=prefix)
    except Exception as ex:
        logger.error('该结果无法生成分级地理图, 异常信息：{}', ex)
