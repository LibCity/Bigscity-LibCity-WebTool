...
def add_choropleth(csv_url, m, state_geo, tag1=None, tag2=None, name="choropleth", tag_prefix=''):
    """
    生成分级图
    :param
    csv_url: csv文件地址
    ...
    if tag2 is None:
        ...
        cp = folium.Choropleth(
            ...
            columns=["geo_id", tag_prefix + tag1],
            key_on="feature.id",
            fill_color="YlGn",
            fill_opacity=0.7,
            line_opacity=0.2,
            legend_name=tag_prefix + tag1,
            highlight=True
        ).add_to(m)
        # 添加分级图 tooltip 显示
        folium.GeoJsonTooltip(fields=['geo_id', tag1],).add_to(cp.geojson)
    else:
        logger.info('two tag provided')
        cp = folium.Choropleth(
            geo_data=state_geo,
            name=name,
            data=choropleth_data,
            columns=["geo_id", tag_prefix + 'total_' + tag1 + '_' + tag2],
            key_on="feature.id",
            fill_color="YlGn",
            fill_opacity=0.7,
            line_opacity=0.2,
            legend_name=tag_prefix + 'total_' + tag1 + '_' + tag2,
            highlight=True
        ).add_to(m)
        # 添加分级图 tooltip 显示
